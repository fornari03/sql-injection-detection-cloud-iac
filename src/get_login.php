...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['username']) && isset($_GET['password'])) {
        ...
        // Mensagem inicial quando a página é acessada sem parâmetros GET
        $login_message = "<div class='info'>Use os parâmetros 'username' e 'password' na URL para testar. Ex: ?username=admin&password=password</div>";
    }
} catch (Exception $e) {
...
<head>
    ...
    <title>Login SQL Injection (Vulnerável - GET)</title>
    <style>
        body {
...
<body>
    <h2>Login Vulnerável a SQL Injection (GET)</h2>
    <p>Este formulário agora processa credenciais via parâmetros na URL para detecção facilitada.</p>
    <form class="login-form" method="GET" action="">
        <label for="username">Usuário</label>
        <input type="text" id="username" name="username" placeholder="Digite seu usuário" required>
        <label for="password">Senha</label>
        <input type="password" id="password" name="password" placeholder="Digite sua senha" required>
        <button type="submit">Entrar (via GET)</button>
    </form>
    <div id="result">
